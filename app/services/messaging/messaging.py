...
class Messaging:
    ...
    def send_whatsapp_text(cls, to_phone: str, text: str, tenant: Optional[str] = None) -> None:
        ...
        try:
            ...
            if sid and not sid.startswith("AC") and "401" in str(e):
                logger.error("Twilio Authentication Error for tenant %s. The account_sid '%s' is likely a Messaging Service SID (starting with HX) instead of an Account SID (starting with AC). Please use your Account SID for authentication.", tenant, sid)
            else:
                logger.error("Twilio error sending text: %s", e)
            raise e

    @classmethod
    def send_whatsapp_media(cls, to_phone: str, media_url: str, caption: Optional[str] = None, tenant: Optional[str] = None) -> None:
        wa_cfg = {}
        if tenant:
            wa_cfg = cls.get_config(tenant).get("whatsapp_config") or {}
    ...
    @classmethod
    def send_whatsapp_interactive(cls, to_phone: str, interactive_payload: Dict[str, Any], tenant: Optional[str] = None) -> None:
        wa_cfg = {}
        if tenant:
            wa_cfg = cls.get_config(tenant).get("whatsapp_config") or {}
        ...
        if not enabled:
            logger.info("[NO-OP] [%s] WhatsApp interactive to %s: %s", tenant or "GLOBAL", to_phone, interactive_payload)
            return
